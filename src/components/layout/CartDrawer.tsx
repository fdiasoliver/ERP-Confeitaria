  ...
  return (
    <>
      <div
        ...
        aria-hidden
      />
      <div className="fixed bottom-0 left-1/2 z-50 w-full max-w-app -translate-x-1/2 rounded-t-3xl bg-white px-5 pb-8 pt-4 shadow-2xl">
        <div className="mx-auto mb-5 h-1 w-10 rounded-full bg-sand" />
        <h2 className="font-display mb-4 text-lg font-semibold">Seu carrinho</h2>
...
export function CartFab({ onClick }: { onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="fixed bottom-6 left-1/2 z-20 flex -translate-x-1/2 items-center gap-2 rounded-full bg-chocolate px-5 py-3.5 font-semibold text-white shadow-lg md:left-auto md:translate-x-0 md:right-[max(1.5rem,calc(50%-28rem))]"
    >
