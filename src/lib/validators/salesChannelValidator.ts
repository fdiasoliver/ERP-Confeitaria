import type { SalesChannelInput, ValidationError } from "@/lib/types";

const HEX_COLOR_RE = /^#[0-9A-Fa-f]{6}$/;

export function validateSalesChannelCreate(input: SalesChannelInput): ValidationError[] {
  ...
  // icon — obrigatório, string não-vazia, máx 50 chars
  if (input.icon === undefined || input.icon.trim() === "") {
    errors.push({ field: "icon", code: "REQUIRED", message: "Ícone é obrigatório." });
...
export function validateSalesChannelUpdate(
  ...
  // icon — se fornecido, não pode ser vazio, máx 50 chars
  if (input.icon !== undefined) {
    if (input.icon.trim() === "") {
      errors.push({ field: "icon", code: "REQUIRED", message: "Ícone não pode ser uma string vazia." });
    } else if (input.icon.trim().length > 50) {
      errors.push({ field: "icon", code: "MAX_LENGTH", message: "Ícone deve ter no máximo 50 caracteres." });
    }
  }
