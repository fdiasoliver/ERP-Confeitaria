...
| **ÉPICO 3** | Inteligência Operacional: Precificação automática avançada (P3.1), Dashboard Executivo — DRE + fluxo de caixa, Relatórios financeiros | 🟡 **Iniciado 07/09/2026.** Módulo P3.1 (Precificação automática) ✅ concluído; Módulo P3.2 (Relatórios financeiros, incluindo DRE e fluxo de caixa) ✅ concluído 13/09/2026 — ver abaixo. Módulos de apoio entregues no mesmo período: Despesas e Canais de Venda (commits `fe5508f`, `563d717`). Sem documento de encerramento do épico |
| **ÉPICO 4** | Expansão Operacional: Calendário de Produção, Usuários/Roles, Módulo Tema, Cálculo de distância, PWA | 🟡 **Itens originais entregues em set/2026:** Usuários (`448ecbc`, 09/09), Calendário de Produção (`75c9f04` 09/09 + reagendamento negociado 15/09, ver P3.3), PWA (`3e5abcc`, 09/09), Módulo Tema (`e4bb212`, 10/09), Cálculo de distância (Módulo 5.A, 07/09). Usuários, Tema e PWA não têm entrada própria em `CHANGELOG.md`. Sem documento de encerramento do épico |
...
| Sprint I.4 | Deploy em produção: Vercel + Supabase + Hostgator — projeto vinculado (`doce-menina/confeitaria-app`, GitHub conectado), variáveis de ambiente de Produção (`DATABASE_URL`, `NEXTAUTH_SECRET`/`URL`, WhatsApp, Supabase Storage), subdomínio `app.confeitariadocemenina.com.br` (registro A na Hostgator), bucket `store-assets` criado | 🟡 **Em andamento — iniciado 04/09/2026, Storage concluído 05/09/2026.** Ver `CHANGELOG.md`. Deploy de produção real no ar, dados reais do Supabase confirmados via smoke test (`/api/config`, `/api/products`, `/admin/login`); push em `main` já aciona redeploy automático via integração GitHub↔Vercel. Upload de imagens (logo/favicon/produto/categoria/banner/cliente/fornecedor) configurado em produção — bucket `store-assets` público criado, `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` no ambiente de Produção. **Pendências:** PIX e Google Maps ainda não configurados em produção (mesmo gap já registrado desde a Sprint I.2); documento de encerramento (`MODULE_I4_CLOSURE.md`) ainda não criado. **Ocorrência de 08/09/2026 (commit `ca639d5`):** produção retornou 500 em todas as rotas por `EMAXCONNSESSION` (`pool_size: 15` do Session Pooler); `DATABASE_URL` passou a apontar para o Transaction Pooler (6543, `?pgbouncer=true`) e `DIRECT_URL` (5432) ficou para `prisma migrate`/`db push` — ver `prisma/schema.prisma` e `.env.example`. A atualização das variáveis na Vercel era manual (Product Owner) e não é verificável pelo repositório |
...
| **UX** — Nota "Não filtrado por período" (Contas a Pagar) sem destaque distinto | Product Review, Sprint 3 (Dashboard Executivo — `/admin/relatorios`), 13/09/2026 | A nota tem o mesmo peso visual das notas de escopo triviais de Fluxo de Caixa/DRE, com risco de o usuário achar que Contas a Pagar também respeita o filtro de período do topo da página. Recomendação: destaque visual distinto (ícone informativo ou tratamento diferente). |
| **UI** — Cor `rose` aplicada a toda linha negativa do DRE, inclusive dedução rotineira | Product Review, Sprint 3 (Dashboard Executivo — `/admin/relatorios`), 13/09/2026 | `rose` é semântica de erro/negativo real em `DESIGN_SYSTEM.md`, mas o DRE a usa também em deduções normais (CMV, despesas operacionais, impostos), que são negativas por definição, não por anomalia. Recomendação: reservar `rose` só para subtotal real negativo (prejuízo); linhas de dedução rotineira em tom neutro. |
| **UX** — Redundância de sinal negativo no DRE (label `"(−)"` + valor já negado) | Product Review, Sprint 3 (Dashboard Executivo — `/admin/relatorios`), 13/09/2026 | O label da linha já traz `"(−)"` e o valor exibido também é negado, duplicando a indicação de subtração. Recomendação: exibir o valor em módulo quando o label já contiver `"(−)"`. |
| **UI** — Separador visual das 3 novas seções diverge das 4 seções pré-existentes de `/admin/relatorios` | Product Review, Sprint 3 (Dashboard Executivo — `/admin/relatorios`), 13/09/2026 | Fluxo de Caixa/Contas a Pagar/DRE usam `border-t border-sand pt-4`, não usado pelas seções anteriores da mesma página — inconsistência de linguagem visual. Recomendação: padronizar (aplicar a todas as seções ou remover das novas). |
| **Melhoria** — Card "Total vencido" reimplementa o layout do `StatCard` em vez de estendê-lo | Product Review, Sprint 3 (Dashboard Executivo — `/admin/relatorios`), 13/09/2026 | O card de Contas a Pagar duplica manualmente a estrutura do `StatCard` compartilhado em vez de receber um tom condicional (`tone?: "default"|"danger"`). Recomendação: promover `tone` a prop do componente compartilhado, já que o padrão tende a se repetir (ex. estoque baixo, margem negativa). |
| **Acessibilidade** — Vencimento em Contas a Pagar indicado só por cor | Product Review, Sprint 3 (Dashboard Executivo — `/admin/relatorios`), 13/09/2026 | Na tabela de Contas a Pagar, a única forma de identificar despesa vencida é a cor do texto da data, sem ícone ou palavra — contraria `DESIGN_SYSTEM.md` ("nunca usar apenas cor para alerta"). Recomendação: adicionar rótulo textual curto ("Vencido") ou ícone junto à data. |
| **UI** — Vitrine (`/`) com layout incorreto | Product Owner, 13/09/2026 | Reportado pelo Product Owner como "layout errado", sem detalhamento ainda de qual elemento/breakpoint está incorreto — a especificação exata do que precisa mudar fica pendente de levantamento numa sessão dedicada antes de qualquer implementação. **Atualização 23/09/2026:** a Vitrine foi redesenhada em 20/09/2026 (commit `b272cb7`, ver `CHANGELOG.md`); não há confirmação do Product Owner de que isso resolve o item reportado. |
| **Acessibilidade** — Contraste de `text-xs text-muted` sobre `cream` nas 3 notas novas (reincidência) | Product Review, Sprint 3 (Dashboard Executivo — `/admin/relatorios`), 13/09/2026 | As 3 notas de transparência novas repetem a mesma limitação de contraste já registrada em `CLAUDE.md` ("A definir" → Acessibilidade) — não é achado isolado novo, reforça a prioridade da pendência já existente. |
| **UX** — Botão "Exportar PDF" habilitado durante carregamento/erro das seções | Product Review, Sprint 3 (Dashboard Executivo — `/admin/relatorios`), 13/09/2026 | O botão permanece habilitado mesmo com Fluxo de Caixa/DRE/Contas a Pagar ainda carregando ou em erro, podendo gerar PDF com skeletons/mensagens de erro. Recomendação: desabilitar o botão enquanto qualquer seção relevante estiver carregando ou com erro. |
| **Melhoria** — Breakpoints de grid dos cards de estatística divergem entre seções da mesma página | Product Review, Sprint 3 (Dashboard Executivo — `/admin/relatorios`), 13/09/2026 | As seções de `/admin/relatorios` usam critérios de coluna diferentes sem justificativa documentada (2/4 colunas vs. 1/3 vs. 1/2). Recomendação: unificar critério em sprint futura de Design System. |

### Entregas fora de Épico/Sprint numerada (15–20/09/2026)

Registro do que foi implementado e commitado sem Sprint Oficial nem ADR. Detalhe de cada item no `CHANGELOG.md` (os marcados com ✔) ou em `git log`.

| Entrega | Data | Commit(s) | `CHANGELOG.md` |
|---|---|---|---|
| Cadastro de cliente (autocadastro em `/cadastro`, endereços salvos reaproveitados no checkout) | 16/09 | `7c63626` | — |
| Cliente corporativo e orçamentos criados pelo admin (`/admin/orcamentos`); ativar/desativar/excluir em `/admin/clientes` | 16/09 | `7d862b8`, `d9eadbf`, `f7ab1b2` | — |
| Link público de aprovação de orçamento + front admin do ciclo de vida do orçamento | 17/09 | `d328a2a` | ✔ |
| Importação/exportação de ingredientes por Excel | 19/09 | `f0577c7` | ✔ |
| Preço por embalagem em ingredientes | 19–20/09 | `f184dc7` | ✔ |
| Duplicar receita (lista e detalhe) | 20/09 | `f5fd661`, `bd325c0` | ✔ |
| Gestão direta de endereços de cliente pela equipe (adicionar/editar/excluir) | 20/09 | `b7508e3`, `e25e3b9` | ✔ |
| Página pública de orçamento redesenhada, orçamentos recusados visíveis, correção de LTV | 20/09 | `e40bd22` | ✔ |
| Editor completo de orçamento | 20/09 | `245491b` | ✔ |
| Redesenho editorial da Vitrine | 20/09 | `b272cb7` | ✔ |
...
| Módulo | Descrição | Dependências | Status |
|--------|-----------|---------------|--------|
| **5.A** | Google Maps (Routes API — `computeRouteMatrix`) — substitui `distanceKm` hardcoded no checkout | Nenhuma | ✅ **Concluído — 07/09/2026** — ver `CHANGELOG.md`. Distance Matrix API está em modo legado (confirmado na documentação oficial); implementado com a Routes API. Checagem em tempo real no checkout + recálculo obrigatório server-side em `POST /api/orders` (pedido fora do raio de `ENTREGA_GRATIS` é rejeitado, nunca confia no cliente). Validado com Playwright + banco real (aviso, bloqueio, rejeição server-side, sem regressão em `RETIRADA`). **Pendência:** teste ponta-a-ponta com distância real depende do Product Owner criar a API key (Google Cloud, faturamento ativo) |
...
| [KNOWN_ISSUES.md](KNOWN_ISSUES.md) | 20 problemas conhecidos (KI-01 a KI-20) com prioridade e arquivos afetados |
...
✅ **Concluído — 13/09/2026** — ver `CHANGELOG.md` e `REGRAS_NEGOCIO.md` Seções 13.3, 13.4, 13.6 e 13.8. Entregue em três sprints, todas em `/admin/relatorios`, atrás de `requireFinance()` (ADMIN+FINANCEIRO): **Sprint 1** — dashboard financeiro (faturamento, ticket médio, CMV, margem bruta), breakdown por categoria de produto e por método de pagamento, exportação CSV por seção e exportação PDF via impressão do navegador; **Sprint 2** — Centro de custo (despesas pagas por canal de venda e por categoria de produto); **Sprint 3** — Fluxo de caixa (realizado, não projetado), Contas a pagar (visão de dívida em aberto, não filtrada por período) e DRE. Escopo do módulo ampliado, durante a execução, para incorporar Centro de custo/Fluxo de caixa/DRE/Contas a pagar — itens já previstos na descrição do ÉPICO 3 como um todo (linha acima), não apenas nas "Entregas" originais desta seção.
...
✅ **Concluído — 15/09/2026** — ver `CHANGELOG.md` e `REGRAS_NEGOCIO.md` Seção 12.11. Aba "Calendário" (visualização por dia/semana, alerta de sobreposição) implementada em commit anterior (`75c9f04`), sem entrada própria no `CHANGELOG.md` até esta sprint (gap pré-existente, não resolvido retroativamente — ver nota na entrada desta sprint no `CHANGELOG.md`). Reagendamento negociado (sugestão do admin + resposta do cliente) implementado e documentado nesta sprint, completando a lista de Entregas abaixo.
