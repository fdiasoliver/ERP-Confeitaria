...
- Usar `.font-display` para títulos (peso 800); o corpo usa peso 400–600 — ambos Manrope, fonte única desde a Sprint `DS.3`
...
| **[ADR-023] Nova camada `src/lib/clients/` para I/O HTTP de serviço externo** | Primeira sprint do Épico 5 (Integrações Externas) — Módulo `5.B`, WhatsApp OTP + Notificações — introduziu `src/lib/clients/whatsappClient.ts`, papel análogo a Repository mas para chamada HTTP a provedor externo (Evolution API), em vez de Prisma. Chamado exclusivamente por Service (`otpService.ts`/`whatsappNotificationService.ts`), nunca por Route Handler diretamente — preserva o fluxo de camadas `Route Handler → Service → Validator → Repository → Prisma`, só adiciona um ramo novo (`Service → Client externo`) sem substituir nenhum já existente. **Motivo:** nenhuma das camadas já documentadas (`architecture`/`references/layers.md`) descreve I/O HTTP para provedor externo — `upload/route.ts` (Supabase Storage) já fazia isso via `fetch` direto na Route, sem Client dedicado, precedente que esta ADR não revoga retroativamente (fora de escopo desta sprint), mas que não deve ser repetido em integrações novas a partir de agora. **Isolamento de risco de contrato externo:** todo o formato específico da API da Evolution API (endpoint, formato de `number`, header `apikey`) fica confinado a `whatsappClient.ts` — nenhum Service/Route conhece esse formato, mitigando o risco de mudança de versão do provedor. **Padrão a replicar nos próximos módulos do Épico 5** (PIX, Google Maps, CONAB/CEASA): cada um ganha seu próprio `src/lib/clients/{provider}Client.ts`, mesmo papel. Documentos impactados: `CLAUDE.md` (esta ADR). `references/layers.md` (skill `architecture`) ainda não atualizado formalmente com a camada nova — achado registrado, pendência para quando a skill for revisada, não bloqueia esta sprint. Aprovado pelo Product Owner nesta sessão. | 05/08/2026 |
| **[ADR-025] Adoção do shadcn/ui como camada de componentes; redesign completo (Sprint `DS.5`)** | Reverte a proibição até então vigente ("Não introduzir bibliotecas de componentes externas... sem decisão explícita") — decisão explícita do Product Owner nesta sessão. **Motivo:** 4 rounds anteriores de redesign (`DS.1`–`DS.4`) trocaram só paleta/tokens/layout, nunca a implementação dos componentes; o projeto nunca teve uma camada de interação testada (focus trap, diálogos acessíveis, variantes consistentes) — tudo escrito à mão em `src/components/`. Diagnóstico registrado nesta sessão: Tailwind CSS não é o limitador, a ausência de uma biblioteca de componentes é. **Escolha:** shadcn/ui (primitivas Radix + Tailwind, código-fonte copiado para o repositório, não é dependência opaca — mas os pacotes `@radix-ui/*` e `sonner` por trás de cada componente usado **entram como dependências reais** em `package.json`, isso não é uma exceção silenciosa à natureza "copiada" do shadcn). **Escopo:** redesign completo (não gradual), admin + cliente, mapeando os 8 tokens já existentes (`cream`/`chocolate`/`rose`/`sage`/`caramel`/`sand`/`muted`/`surface-2`) para as variáveis semânticas do shadcn (`--background`/`--foreground`/`--primary`/`--destructive`/etc.) — nenhuma cor nova introduzida. **Decisões de escopo já tomadas nesta sessão, não delegadas à execução:** toasts usam `sonner` (padrão atual do shadcn, substitui `Toast`/`ValidationSummary` bespoke); fundos "white" hoje literais em card/modal são consolidados em `surface-2`/`cream` (não viram um 9º token); `sage`/`caramel` continuam como utilitário Tailwind solto sobre os componentes shadcn, sem slot semântico próprio (`--success`/`--warning`); arquivos com múltiplas exportações relacionadas (`Header.tsx`, `CartDrawer.tsx`, `ProductCard.tsx`) permanecem mesclados, não forçados ao padrão 1-componente-por-arquivo do shadcn; `FormPrimitives.tsx` (`Field`/`Section`, reuso obrigatório por `PROJECT_GOVERNANCE.md` §8.5) mantém a mesma API pública, só o JSX interno muda. Storybook/tooling adicional fora de escopo. **Numeração:** `DS.5`, seguindo o prefixo já reservado em ADR-018. Documentos impactados: `CLAUDE.md` (esta ADR + correção da linha stale "Fraunces/DM Sans", já substituída por Manrope desde a Sprint `DS.3`, 10/08/2026 — nunca atualizada na tabela de stack), `DESIGN_SYSTEM.md`, `UX_GUIDELINES.md`, `PLAN.md`, `CHANGELOG.md`. Plano completo em sessão dedicada, aprovado pelo Product Owner. | 27/08/2026 |
| **[ADR-024] Área cliente responsiva: route group `(client)`, `ClientShell`, `VitrineSidebar` própria, breakpoint `md`** | Sprint `DS.4` — Vitrine/Checkout/Pedidos/Login (antes soltas em `src/app/`) movidas para `src/app/(client)/`, mesmo padrão de agrupamento já usado pelo admin (`src/app/admin/`), sem alterar nenhuma URL (route group do Next.js). Novo `src/components/layout/ClientShell.tsx` (paralelo a `AdminShell.tsx`, DS.2) via `src/app/(client)/layout.tsx` — hoje responsável por hospedar `CartFab`/`CartDrawer`/`Toast` e sincronizar `?cart=open`. Nova `src/components/vitrine/VitrineSidebar.tsx`, recolhível (modo só-ícone, `useState` local, sem persistência) — **componente próprio, não reaproveita** `src/components/admin/shared/Sidebar.tsx`: fonte de dados distinta (ocasiões públicas via `/api/occasions`, não sessão/papel). Ícones reais via `lucide-react` (biblioteca nova no projeto), mapeados em `src/lib/icons.ts` a partir de `OccasionTag.icon` (campo já existente no schema, até então sempre `"calendar"` — atualizado nesta sprint com um ícone distinto por ocasião real). **Breakpoint `md` (768px), decisão explícita do Product Owner, não a recomendação original do `ai-solution-architect` (que era `lg`/1024px):** mantém paridade exata com o breakpoint já usado pela Sidebar do admin (DS.2), em vez de um breakpoint novo só para a área cliente. **Carrinho disponível em todas as páginas cliente, decisão explícita do Product Owner, não a recomendação original (que era só Vitrine/Pedidos):** `CartFab`/`CartDrawer` centralizados no `ClientShell` em vez de cada página renderizar os seus — `CartDrawer` ganhou variante desktop (`md:`+: painel ancorado à direita, altura cheia, `w-96`) mantendo o bottom-sheet mobile inalterado. **Exceção encontrada durante a validação funcional, não prevista no planejamento inicial:** o Checkout já exibe o carrinho inline (itens + resumo) — com o `CartFab` global, em mobile ele sobrepunha o botão final "Confirmar e pagar com PIX", bloqueando o toque; corrigido ocultando o `CartFab` especificamente em `/checkout` (`usePathname()` no `ClientShell`), decisão validada com o Product Owner nesta sessão. Checkout ganhou layout de 2 colunas em `lg:`+ (formulário à esquerda, resumo do pedido `sticky` à direita); Pedidos ganhou grid `lg:grid-cols-2`; Login permanece estreito (`max-w-app`) mas passou a centralizar verticalmente (`flex` + `justify-center`) em vez de ficar ancorado ao topo com vazio abaixo. Documentos impactados: `CLAUDE.md` (esta ADR), `UX_GUIDELINES.md` Seções 14–16, `PLAN.md`, `CHANGELOG.md`. Aprovado pelo Product Owner nesta sessão. | 11/08/2026 |
| **[ADR-026] Achado registrado: ambiente de produção real (Vercel) diverge do declarado na ADR-010 (VPS Linux)** | **Não resolve a contradição — só a registra**, mesmo padrão de transparência de ADR-006/007/008/009. **ADR-010** (`PROJECT_GOVERNANCE.md` Seção 8.8/11.1, 16/07/2026) declara: *"ambiente oficial de produção é VPS Linux"*. A **Sprint I.4** (04–06/09/2026, ver `CHANGELOG.md`) implementou e validou em produção real um deploy completo na **Vercel** (`app.confeitariadocemenina.com.br`, projeto `doce-menina/confeitaria-app`, Supabase como banco, Hostgator só DNS) — smoke test real confirmado (`/`, `/api/config`, `/api/products`, `/admin/login` todos HTTP 200 com dado real do banco), redeploy automático via GitHub↔Vercel confirmado funcionando. **Isso é uma divergência real entre documento e prática, não uma opinião** — o ambiente que está de fato no ar hoje é Vercel, não um VPS Linux, e nenhuma ADR reconciliou isso até agora. **Origem do achado:** pergunta direta do Product Owner sobre restrições de design/layout do sistema, nesta mesma sessão — ao levantar decisões arquiteturais relevantes (ADRs de runtime/hospedagem) para responder com precisão, a contradição com a Sprint I.4 (já implementada nesta mesma conversa) ficou evidente. **Por que não resolvida aqui:** a Regra de Imutabilidade (`PROJECT_GOVERNANCE.md` Seção 13.5) exige uma ADR nova, explicitamente aprovada pelo Product Owner, para reverter ou superar uma ADR existente — não é uma correção editorial que a IA possa fazer sozinha. **Decisão pendente do Product Owner:** (a) revogar/atualizar a ADR-010 para declarar Vercel como ambiente oficial, (b) tratar o deploy da Sprint I.4 como estágio temporário até um VPS Linux real ser provisionado, ou (c) outra reconciliação. Documentos impactados por esta ADR: `CLAUDE.md` (só esta entrada), `ARCHITECTURE.md` (Seção 6, "Decisões já tomadas"). **Fora do escopo desta ADR, intocado:** `PROJECT_GOVERNANCE.md` Seção 8.8/11.1 (texto original da ADR-010 permanece como está, por decisão explícita de não alterá-lo sem a reconciliação formal do Product Owner). | 06/09/2026 |
